...
For the expanded, step-ordered “prompt-to-prompt” resource, see:
...
2) **Pick ONE implementer lane**
   ...
   - If Jules should run from GitHub, also add the `jules` label to the Issue.
3) **Plan in ChatGPT**
   - Get a small plan + touch list + test plan.
4) **Red-team in Gemini**
   - Try to break the plan; add missing edge cases.
5) **Implement (default: Jules)**
   - Use Jules for most work (one Issue -> one PR).
   - Use Cursor only for local debug / last-mile fixes.
6) **You verify**
